...
import { Search, Loader2, X, Menu, MessageCircle } from "lucide-react";
      ...
      <nav className="mx-[12px] mt-[12px] md:m-[24px] bg-[#141414] border border-[#282828] rounded-[12px] h-[56px] md:h-[64px] px-[12px] md:px-[16px] flex items-center justify-between gap-[12px] md:gap-[32px]">
        ...
        <div className="hidden md:flex items-center gap-[32px] shrink-0">
          <div className="flex items-center gap-[32px]">
            {[{ label: "Profile", href: "/profile" }, { label: "Settings", href: "/settings" }].map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`text-sm font-medium transition-colors whitespace-nowrap ${
                  pathname === item.href ? "text-[#FFFFFF]" : "text-[#A3A3A3] hover:text-[#FFFFFF]"
                }`}
              >
                {item.label}
              </Link>
          ...
          <Button variant="primary" size="sm" asChild>
            <Link href="/discord" className="whitespace-nowrap">Discord</Link>
          </Button>
        </div>
        ...
        <div className="flex md:hidden items-center gap-[8px]">
          ...
          <button
            ...
            onClick={() => { setShowMobileSearch(prev => !prev); setShowMobileMenu(false); }}
            ...
            aria-label="Search"
          >
            {showMobileSearch ? <X className="w-[16px] h-[16px]" /> : <Search className="w-[16px] h-[16px]" />}
          </button>
          ...
          <Link
            ...
            aria-label="Discord"
          >
            <svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="w-[18px] h-[18px]">
          ...
          {/* Hamburger */}
          <button
            onClick={() => { setShowMobileMenu(prev => !prev); setShowMobileSearch(false); }}
            className="h-[36px] w-[36px] flex items-center justify-center rounded-[8px] bg-[#242424] border border-[#282828] text-[#A3A3A3] hover:text-white transition-colors"
            aria-label="Menu"
          >
            {showMobileMenu ? <X className="w-[16px] h-[16px]" /> : <Menu className="w-[16px] h-[16px]" />}
          </button>
        </div>
      </nav>
            ...
            {query.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-[8px] bg-[#141414] border border-[#282828] rounded-[12px] shadow-2xl overflow-hidden z-50">
                {isSearching ? (
                  ...
                  <div className="flex flex-col p-[8px] gap-[4px]">
                    {searchResults.map((item: any) => (
                      <Link
                        href={`/anime/${item.id}`}
                        key={item.id}
                        onClick={() => { setIsFocused(false); setShowMobileSearch(false); }}
                        className="flex items-start gap-[12px] p-[8px] rounded-[8px] hover:bg-[#242424] transition-colors"
                      >
                        <div className="relative aspect-[2/3] w-[40px] rounded-[6px] bg-[#141414] overflow-hidden shrink-0 border border-[#282828]">
                          {item.posterImage && <img src={item.posterImage} alt={item.title} className="w-full h-full object-cover" />}
                        </div>
                        <div className="flex flex-col min-w-0 flex-1 pt-[2px]">
                          <span className="text-base font-bold text-[#FFFFFF] truncate leading-tight">{item.title}</span>
                          <div className="flex flex-wrap items-center gap-[6px] mt-[4px]">
                            {item.year && <span className="text-[#A3A3A3] font-bold bg-[#242424] border border-[#282828] px-[6px] py-[2px] rounded text-[9px]">{item.year}</span>}
                            {item.format && <span className="text-[#A3A3A3] font-bold bg-[#242424] border border-[#282828] px-[6px] py-[2px] rounded text-[9px] uppercase">{item.format}</span>}
                            <span className="text-emerald-400 font-bold bg-emerald-500/10 border border-emerald-500/20 px-[6px] py-[2px] rounded text-[9px]">{item.score} ★</span>
                          </div>
                        </div>
                      </Link>
                    ))}
                  ...
                  <div className="p-[16px] text-center text-sm text-[#A3A3A3]">No results for "{query}"</div>
                )}
              </div>
            )}
          </div>
              ...
              return (
                <Link
                  ...
                  className={`px-[12px] py-[12px] rounded-[8px] text-sm font-bold transition-colors ${
                    isActive ? "text-[#FFFFFF] bg-[#242424]" : "text-[#A3A3A3] hover:text-[#FFFFFF] hover:bg-[#242424]/60"
                  }`}
                >
                  {item.label}
                </Link>
