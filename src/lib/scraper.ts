...
export async function fetchAnimeEpisodes(anilistId: string) {
  ...
  const epMap = new Map<number, unknown>();

  for (const [providerId, providerData] of Object.entries(data)) {
    // softsubEps: Solaris/Frost — HLS + separate VTT track (toggle off/on)
    const softsubEps =
      (providerData as Record<string, unknown>)?.episodes?.softsub || [];
    // subEps: hard-sub providers — subs burned into the video
    const subEps =
      (providerData as Record<string, unknown>)?.episodes?.sub || [];
    const dubEps =
      (providerData as Record<string, unknown>)?.episodes?.dub || [];
