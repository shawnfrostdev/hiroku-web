...
const CONTINUE_PLAYING_FALLBACK = [
  {
    animeId: "154587",
    animeTitle: "Frieren: Beyond Journey's End",
    animeImage: "https://s4.anilist.co/file/anilistcdn/media/anime/banner/154587-n6ca655rBLSj.jpg",
    episode: "Episode 18",
    progress: 75,
    timeLeft: "6m left",
  },
  {
    ...
    animeImage: "https://s4.anilist.co/file/anilistcdn/media/anime/banner/145064-9K642C978D7o.jpg",
    episode: "Episode 12",
    progress: 40,
    timeLeft: "14m left",
  },
  {
    ...
    animeImage: "https://s4.anilist.co/file/anilistcdn/media/anime/banner/136003-75T9a4H0R9kD.jpg",
    episode: "Episode 8",
    progress: 90,
    timeLeft: "2m left",
  },
...
const TAG_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  SPORTS: { bg: "bg-lime-500/10", text: "text-lime-400", border: "border-lime-500/20" },
  MYSTERY: { bg: "bg-violet-500/10", text: "text-violet-400", border: "border-violet-500/20" },
  SLICE_OF_LIFE: { bg: "bg-sky-500/10", text: "text-sky-400", border: "border-sky-500/20" },
  "SLICE OF LIFE": { bg: "bg-sky-500/10", text: "text-sky-400", border: "border-sky-500/20" },
  THRILLER: { bg: "bg-fuchsia-500/10", text: "text-fuchsia-400", border: "border-fuchsia-500/20" },
  SUPERNATURAL: { bg: "bg-indigo-500/10", text: "text-indigo-400", border: "border-indigo-500/20" },
  TRENDING: { bg: "bg-amber-500/10", text: "text-amber-400", border: "border-amber-500/20" },
  POPULAR: { bg: "bg-emerald-500/10", text: "text-emerald-400", border: "border-emerald-500/20" },
  UPCOMING: { bg: "bg-blue-500/10", text: "text-blue-400", border: "border-blue-500/20" },
  ANIME: { bg: "bg-neutral-500/10", text: "text-neutral-400", border: "border-neutral-500/20" },
};
...
export default function Page() {
  ...
  const { data: popularSeason = DEFAULT_ENTRIES, isLoading: isLoadingPopularSeason } = useQuery({
    queryKey: ["popularSeasonAnime"],
    queryFn: fetchPopularSeasonAnime,
  });


  const { data: popularAllTime = DEFAULT_ENTRIES, isLoading: isLoadingPopularAllTime } = useQuery({
    queryKey: ["popularAllTimeAnime"],
    queryFn: fetchPopularAllTimeAnime,
  });

  const { data: finishedMovies = DEFAULT_OBJECT, isLoading: isLoadingFinishedMovies } = useQuery({
    queryKey: ["finishedMovies"],
    queryFn: fetchFinishedMovies,
  });

  const { data: recentlyUpdated = DEFAULT_ENTRIES, isLoading: isLoadingRecentlyUpdated } = useQuery({
  ...
  const heroEntries = entries && Array.isArray(entries) ? entries.slice(0, 5) : [];
  const [activeIndex, setActiveIndex] = useState(0);
  const [progress, setProgress] = useState(0);
  const activeEntry = heroEntries[activeIndex];
  const [forYouEntries, setForYouEntries] = useState<any[]>([]);
  ...
  }, [activeIndex, heroEntries.length]);
  ...
  if (isLoading || isLoadingPopularSeason || isLoadingPopularAllTime || isLoadingFinishedMovies || isLoadingRecentlyUpdated) {
    return (
      <main className="flex-1 flex flex-col gap-[24px] px-[24px] pb-[24px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px] mt-[16px] w-full">
          ...
          <div className="flex flex-col gap-[16px]">
            ...
            <div className="flex flex-col gap-[12px]">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex items-center gap-[16px] animate-pulse">
        ...
        <div className="flex flex-col gap-[16px] mt-[16px]">
          ...
          <div className="flex flex-nowrap overflow-x-hidden gap-[16px] w-full py-[8px]">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="flex flex-col gap-[8px] p-[8px] -m-[8px] shrink-0 w-[120px] md:w-[160px] animate-pulse">
  ...
  if (!activeEntry) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-[24px] text-center gap-[16px] min-h-[50vh]">
        <h2 className="text-xl font-bold text-[#FFFFFF]">Failed to load anime catalog</h2>
        <p className="text-sm text-[#A3A3A3] max-w-md">
          There was an error communicating with the anime service or the catalog is empty. Please check your internet connection or try reloading.
        </p>
    ...
    <main className="flex-1 flex flex-col pt-[12px] md:pt-0 gap-[12px] md:gap-[24px] px-[12px] md:px-[24px] pb-[24px]">
        ...
        >
              ...
              <div className="w-full h-full">
                ...
                {activeEntry?.posterImage && (
                  <div
                    ...
                    style={{ backgroundImage: `url(${activeEntry.posterImage})` }}
                  />
                )}
                ...
                {activeEntry?.bannerImage && (
                  <div
                    ...
                    style={{ backgroundImage: `url(${activeEntry.bannerImage})` }}
                  />
                )}
                ...
                <div
                  ...
                  style={{
                    background: "linear-gradient(to top, rgba(20, 20, 20, 1) 0%, rgba(20, 20, 20, 0.8) 30%, rgba(20, 20, 20, 0.2) 70%, rgba(20, 20, 20, 0) 100%)"
                  }}
                />
              </div>
            </motion.div>
          ...
          <div className="relative z-10 flex flex-col gap-[16px] md:gap-[24px]">
            ...
            <div className="flex flex-wrap items-center gap-[8px] md:gap-[12px] mt-[6px] md:mt-[8px]">
              ...
              <Button variant="secondary" size="md" className="gap-[8px]" asChild>
                <Link href={`/anime/${activeEntry.id}`}>
              ...
              <Button
                variant={watchlist[String(activeEntry.id)] ? "primary" : "outline"}
                size="icon"
                onClick={() => {
                  const isBookmarked = !!watchlist[String(activeEntry.id)];
                  if (isBookmarked) {
                    useWatchlistStore.getState().removeItem(String(activeEntry.id));
                  } else {
              ...
              >
                <Bookmark className={`h-[16px] w-[16px] ${watchlist[String(activeEntry.id)] ? "fill-current" : ""}`} />
              </Button>
            </div>
          </div>
        ...
        <div className="lg:hidden flex items-center justify-center gap-[8px]">
          {heroEntries.map((_, idx) => (
            <button
              ...
              style={{ width: idx === activeIndex ? 24 : 8, background: idx === activeIndex ? '#FFFFFF' : '#282828' }}
            ...
            {heroEntries.map((entry, idx) => {
              ...
              return (
                <button
                  ...
                  key={entry.id}
                  onClick={() => setActiveIndex(idx)}
                  className={`relative overflow-hidden w-full text-left p-[16px] border rounded-[8px] transition-all flex flex-col justify-between lg:flex-1 cursor-pointer focus-visible:outline-none ${isActive
                  ...
                  {entry.bannerImage && (
                    ...
                    >
                      <div
                        className="absolute inset-0"
                        style={{
                          background: "linear-gradient(to right, rgba(20, 20, 20, 0.95) 0%, rgba(20, 20, 20, 0.7) 50%, rgba(20, 20, 20, 0.2) 100%)"
                        }}
                      />
                    </div>
                  )}
        ...
        <div className="flex md:grid md:grid-cols-3 lg:grid-cols-5 gap-[8px] md:gap-[16px] overflow-x-auto md:overflow-x-visible snap-x snap-mandatory scrollbar-none pb-[4px] md:pb-0">
          {watchingItems.length > 0
                  ...
                  {item.animeImage && (
                    ...
                    >
                      <div
                        className="absolute inset-0"
                        style={{
                          background: "linear-gradient(to top, rgba(20, 20, 20, 0.95) 0%, rgba(20, 20, 20, 0.6) 50%, rgba(20, 20, 20, 0.1) 100%)"
                        }}
                      />
                    </div>
                  )}
            ...
            : forYouEntries.map((item, index) => {
              const tags = [
                { text: "RECOMMENDED", colorClass: "text-[#FFFFFF] bg-[#10b981] border-[#10b981]" },
                { text: "TRENDING #1", colorClass: "text-[#FFFFFF] bg-[#f97316] border-[#f97316]" },
                { text: "MUST WATCH", colorClass: "text-[#FFFFFF] bg-[#a855f7] border-[#a855f7]" },
                { text: "FAN FAVORITE", colorClass: "text-[#FFFFFF] bg-[#3b82f6] border-[#3b82f6]" },
                { text: "HOT RELEASE", colorClass: "text-[#FFFFFF] bg-[#ef4444] border-[#ef4444]" },
              ];
              ...
              return (
                ...
                >
                  <div
                    className="group relative h-[110px] md:h-[140px] rounded-[10px] border border-[#282828] bg-[#141414] overflow-hidden cursor-pointer hover:border-[#FFFFFF]/60 transition-all flex flex-col justify-between p-[12px] md:p-[16px] shrink-0 snap-start w-[65vw] sm:w-[45vw] md:w-auto"
                  >
                    {item.bannerImage && (
                      <div
                        className="absolute inset-0 bg-cover bg-center transition-transform duration-300 group-hover:scale-105 pointer-events-none"
                    ...
                    <div className="absolute top-[16px] left-[16px] right-[16px] flex items-center justify-between gap-[8px] pointer-events-none">
                      <span className={`text-[9px] font-black px-[7px] py-[3px] rounded-[4px] border uppercase tracking-wide shadow-md ${activeTag.colorClass}`}>
                        {activeTag.text}
                      </span>
                      <span className="text-[10px] text-[#A3A3A3] font-bold bg-[#141414]/90 px-[6px] py-[2px] rounded border border-[#282828] backdrop-blur-[2px]">
                        {item.score} ★
      ...
      <div className="flex flex-col gap-[12px] pt-[16px] border-t border-[#1c1c1c]">
        <div className="flex items-center justify-between">
          ...
          <a href="#" className="text-[11px] font-bold uppercase tracking-wider text-[#A3A3A3] hover:text-[#FFFFFF] transition-colors cursor-pointer">
            View All
          </a>
        </div>
        <div className="flex md:grid md:grid-cols-5 lg:grid-cols-10 overflow-x-auto md:overflow-x-visible snap-x snap-mandatory scrollbar-none gap-[8px] md:gap-[16px] pb-[4px] md:pb-0">
          {recentlyUpdated.slice(0, 10).map((item: any) => (
            <Link
              href={`/anime/${item.id}`}
              key={item.id}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px] pt-[16px] border-t border-[#1c1c1c] w-full">
        ...
        <div className="flex flex-col gap-[16px]">
          ...
          <div className="flex flex-col gap-[12px]">
            {finishedMovies.finished.map((item: any) => (
              <Link
                href={`/anime/${item.id}`}
                key={item.id}
              ...
              >
                ...
                {item.bannerImage && (
                  ...
                  >
                    <div
                      className="absolute inset-0"
                      style={{
                        background: "linear-gradient(to right, rgba(20, 20, 20, 1) 0%, rgba(20, 20, 20, 0.4) 40%, transparent 100%)"
                      }}
                    />
                  </div>
                )}
                ...
                <div className="relative aspect-[2/3] w-[48px] rounded-[6px] border border-[#282828] bg-[#141414] overflow-hidden shrink-0 shadow-md z-10">
                  {item.posterImage ? (
                    <img src={item.posterImage} alt={item.title} className="w-full h-full object-cover transform-gpu will-change-transform" />
                  ) : (
        ...
        <div className="flex flex-col gap-[16px]">
          ...
          <div className="flex flex-col gap-[12px]">
            {finishedMovies.movies.map((item: any) => (
              <Link
                href={`/anime/${item.id}`}
                key={item.id}
                className="group isolate relative overflow-hidden flex items-center gap-[16px] p-[12px] rounded-[8px] hover:bg-[#242424]/40 transition-colors cursor-pointer border border-transparent hover:border-[#282828]"
...
function ScheduleSection() {
  ...
  const days = Array.from({ length: 14 }).map((_, index) => {
    ...
    const releases = scheduleData.filter((item: any) => item.airingAt >= startOfDay && item.airingAt < endOfDay);
          ...
          : days.map((day) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[64px] md:min-w-[72px] py-8px rounded-lg border transition-all shrink-0 ${isDisabled
              ...
              >
                <span className={`text-[10px] font-bold uppercase tracking-wider ${isActive ? "text-text-muted" : ""}`}>
                  {day.label}
                </span>
      ...
      <div className="relative group/carousel w-full mt-4px">
        ...
        {!isLoading && activeReleases.length > 0 && (
          <button
            ...
            className={`absolute left-[8px] md:left-12px top-1/2 -translate-y-1/2 z-20 p-8px bg-surface/90 backdrop-blur-md border border-border-line rounded-full text-text-primary transition-all shadow-xl ${!canScrollLeft ? "opacity-30 cursor-not-allowed" : "cursor-pointer hover:bg-control hover:scale-110"
              }`}
        ...
        {!isLoading && activeReleases.length > 0 && (
          <button
            ...
            className={`absolute right-[8px] md:right-12px top-1/2 -translate-y-1/2 z-20 p-8px bg-surface/90 backdrop-blur-md border border-border-line rounded-full text-text-primary transition-all shadow-xl ${!canScrollRight ? "opacity-30 cursor-not-allowed" : "cursor-pointer hover:bg-control hover:scale-110"
              }`}
        ...
        >
          {isLoading ? (
            ...
            Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex flex-col gap-8px p-8px -m-8px shrink-0 w-[120px] md:w-[160px]">
            ...
            activeReleases.map((item: any) => {
              const releaseDate = new Date(item.airingAt * 1000);
              const timeString = releaseDate.toLocaleTimeString("en-US", { hour: 'numeric', minute: '2-digit' });
